"""
AI Interpreter - Gemini API Wrapper (OpenAI-compatible SDK)
=======================================================
...
from config.settings import GEMINI_API_KEY, GEMINI_MODEL, GEMINI_BASE_URL
...
SYSTEM_PROMPT_CONSULTATION = """Kamu adalah konsultan energi personal yang hangat, empatik, and mendalam.
...
def build_consultation_prompt(data: dict, masalah: str, chat_history: str = "") -> str:
    ...
    Jawab dengan empati, penjelasan energi, and 3-5 saran KONKRET. Jangan pernah sebut istilah teknis."""
...
async def generate_free_reading(data: dict) -> str:
    ...
    try:
                response = await _get_client().chat.completions.create(
                                model=GEMINI_MODEL,
                                messages=[
                                                    {"role": "system", "content": SYSTEM_PROMPT_FREE_READ},
                                                    {"role": "user", "content": build_free_read_prompt(data)}
                                ],
                                temperature=0.8
                )
        ...
        import logging
        logging.getLogger(__name__).error(f"Error AI free reading: {e}", exc_info=True)
        return (
...
async def generate_consultation_reply(data: dict, masalah: str, chat_history: str = "") -> str:
    ...
    try:
                response = await _get_client().chat.completions.create(
                                model=GEMINI_MODEL,
                                messages=[
                                                    {"role": "system", "content": SYSTEM_PROMPT_CONSULTATION},
                                                    {"role": "user", "content": build_consultation_prompt(data, masalah, chat_history)}
                                ],
                                temperature=0.8
                )
                return response.choices[0].message.content
        ...
        import logging
        logging.getLogger(__name__).error(f"Error AI consultation: {e}", exc_info=True)
        return (
